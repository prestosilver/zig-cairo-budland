...
pub const Layout = struct {
    ...
    pub fn getPixelSize(self: *Self) Size {
        ...
        return Size{ .width = @floatFromInt(width), .height = @floatFromInt(height) };
    }
    ...
    pub fn setText(self: *Self, text: []const u8) void {
        c.pango_layout_set_text(self.c_ptr, text.ptr, @intCast(text.len));
    }
...
pub const FontDescription = struct {
    ...
    pub fn getSize(self: *Self) usize {
        return @intCast(c.pango_font_description_get_size(self.c_ptr));
    }
...
pub const Rectangle = struct {
    ...
    pub fn new(x: c_int, y: c_int, width: c_int, height: c_int) !Self {
        ...
        const c_ptr: ?*c.struct__PangoRectangle = @ptrCast(&rect);
