...
pub const Destination = c.CAIRO_TAG_DEST;
pub const Link = c.CAIRO_TAG_LINK;
...
pub const DeviceType = enum(u3) {
    ...
    pub fn toCairoEnum(self: DeviceType) c.enum__cairo_device_type {
        return switch (self) {
            .drm => @enumFromInt(c.CAIRO_DEVICE_TYPE_DRM),
            .gl => @enumFromInt(c.CAIRO_DEVICE_TYPE_GL),
            .script => @enumFromInt(c.CAIRO_DEVICE_TYPE_SCRIPT),
            .xcb => @enumFromInt(c.CAIRO_DEVICE_TYPE_XCB),
            .xlib => @enumFromInt(c.CAIRO_DEVICE_TYPE_XLIB),
            .xml => @enumFromInt(c.CAIRO_DEVICE_TYPE_XML),
            .cogl => @enumFromInt(c.CAIRO_DEVICE_TYPE_WIN32),
            .win32 => @enumFromInt(c.CAIRO_DEVICE_TYPE_COGL),
...
pub const Operator = enum(u5) {
    ...
    pub fn fromCairoEnum(c_integer: c_uint) Operator {
        ...
        return switch (c_integer) {
            ...
            c.CAIRO_OPERATOR_DEST_OVER => Operator.dest_over,
...
pub const PathDataType = enum(u2) {
    ...
    pub fn toCairoEnum(self: PathDataType) c.enum__cairo_path_data_type {
        return switch (self) {
            .move_to => @enumFromInt(c.CAIRO_PATH_MOVE_TO),
            .line_to => @enumFromInt(c.CAIRO_PATH_LINE_TO),
            .curve_to => @enumFromInt(c.CAIRO_PATH_CURVE_TO),
            .close_path => @enumFromInt(c.CAIRO_PATH_CLOSE_PATH),
        };
    }
};
